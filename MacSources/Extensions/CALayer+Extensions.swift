...
extension CALayer {
    @objc func tint(withColors colors: [NSColor]) {
...
extension CAGradientLayer {
    override func tint(withColors colors: [NSColor]) {
        recursiveSearch(inArray: skeletonSublayers,
...
public extension CALayer {
    ...
    var pulse: CAAnimation {
        ...
        pulseAnimation.toValue = NSColor(cgColor: backgroundColor!)!.complementaryColor.cgColor
    ...
    var sliding: CAAnimation {
        ...
        startPointAnim.fromValue = NSValue(point:NSPoint(x: -1, y: 0.5))
        startPointAnim.toValue = NSValue(point:NSPoint(x:1, y: 0.5))
        
        let endPointAnim = CABasicAnimation(keyPath: #keyPath(CAGradientLayer.endPoint))
        endPointAnim.fromValue = NSValue(point:NSPoint(x: 0, y: 0.5))
        endPointAnim.toValue = NSValue(point:NSPoint(x:2, y: 0.5))
